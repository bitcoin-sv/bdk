...
var importantBlocks = map[string][]int32{
	"main": {
		227930,
...
func GetMandatoryBlocks(network string) []int32 {
...
func GetBlockByHeight(api *APIClient, h int32) (string, error) {
...
func GetTxHexExtended(api *APIClient, txID string) (string, []int32, error) {
	...
	parentTxsHeight := make(map[string]int32)
...
func GetChainTip(api *APIClient) (int32, error) {
	if jsonStr, err := GetBlockChainInfo(api); err != nil {
		return int32(0), err
	} else {
		var data partChainInfoBlockHeight
		if err := json.Unmarshal([]byte(jsonStr), &data); err != nil {
			return int32(0), fmt.Errorf("unable to parse the json body, error %w", err)
		} else {
...
// See : https://docs.taal.com/core-products/whatsonchain/block#get-by-height
func GetListTxFromBlock(api *APIClient, blockHeight int32) ([]string, error) {
