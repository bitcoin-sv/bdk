...
std::vector<uint8_t> MakeSig (CScript& script, const CKey &key,CMutableTransaction& spendTx,
...
int main(int argc, char * argv[]){
    ...
    // COnstruct a tx
    CScript scriptSig ; 
    scriptSig << OP_DUP << OP_HASH160 << ToByteVector(pubkey1.GetID())
    ...
    if(DecodeHexTx(rebuilttx, hextx)){
        ...
        uint256 hash = SignatureHash(scriptToHash,stx, 0,
