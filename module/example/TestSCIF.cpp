...
int main(int argc, char * argv[]){
    ...
    if(ScriptEngineIF::executeScript(direct) == SCRIPT_ERR_OK)
        std::cout << "Sucessful script execution" << std::endl;


    std::string scriptString("0x00 0x6b 0x54 0x55 0x93 0x59 0x87");
    if(ScriptEngineIF::executeScript(scriptString) == SCRIPT_ERR_OK )
    ...
    if(ScriptEngineIF::executeScript(scriptHashExample) == SCRIPT_ERR_OK)
    ...
    if(ScriptEngineIF::executeScript(scriptHash)  == SCRIPT_ERR_OK )
