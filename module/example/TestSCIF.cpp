...
int main(int argc, char * argv[]){
    ...
    if(bsv::executeScript(direct,concensus, scriptsflags, std::string(),0,0) == SCRIPT_ERR_OK)
        std::cout << "Sucessful script execution" << std::endl;


    std::string scriptString("0x00 0x6b 0x54 0x55 0x93 0x59 0x87");

    if(bsv::executeScript(scriptString,concensus, scriptsflags, std::string(),0,0) == SCRIPT_ERR_OK )
    ...
    if(bsv::executeScript(scriptHashExample,concensus, scriptsflags,std::string(),0,0) == SCRIPT_ERR_OK)
    ...
    if(bsv::executeScript(scriptHash,concensus, scriptsflags,std::string(),0,0)  == SCRIPT_ERR_OK )
    ...
    try{
        if(bsv::executeScript(scriptVal,concensus, scriptsflags,std::string(),0,0) == SCRIPT_ERR_OK )
