...
BOOST_AUTO_TEST_CASE(script_status)
{
    ...
    {
        const auto status{evaluate(script, true, 0, std::string{}, 0, 0)};
...
BOOST_AUTO_TEST_CASE(test_case_1)
{
    ...
    BOOST_CHECK_EQUAL(bsv::evaluate(script_string, true, 0, std::string(), 0, 0), SCRIPT_ERR_OK);

    // 'abc...', OP_HASH256, OP_PUSHDATA1, 20, '20 data bytes', OP_EQUALVERIFY
    std::string script_hash_example("'abcdefghijklmnopqrstuvwxyz' 0xaa 0x4c 0x20 "
                                    "0xca139bc10c2f660da42666f72e89a225936fc60f19"
                                    "3c161124a672050c434671 0x88");
    BOOST_CHECK_EQUAL(bsv::evaluate(script_hash_example, true, 0, std::string(), 0, 0), SCRIPT_ERR_OK);

    std::string script_hash("'abcdefghijklmnopqrstuvwxyz' OP_HASH256 OP_PUSHDATA1 0x20 "
                            "0xca139bc10c2f660da42666f72e89a225936fc60f193c161124a672050c434671 "
                            "OP_EQUALVERIFY");
    BOOST_CHECK_EQUAL(bsv::evaluate(script_hash, true, 0, std::string(), 0, 0), SCRIPT_ERR_OK);
}
...
BOOST_AUTO_TEST_CASE(test_case_2)
{
    ...
    const std::string& fullscript = "0x47 "
    ...
    const std::string& txhex = "0100000001d92670dd4ad598998595be2f1bec959de9a9f8b1fd97fb832965c96cd551"
                               "45e20000000000ffffffff010a000000000000000000000000";

    int64_t amt = 10;

    BOOST_CHECK_EQUAL(bsv::evaluate(fullscript, true, 0, txhex, 0, amt), SCRIPT_ERR_OK);
}
