...
endfunction()##bdkFindBSVDir

function(bdkSetMinimumListBSVSource)########################################################################################
  ...
  set(_minimal_hdr_files
      ...
      "src/script/shiftnum.h"  ##  Used by [interpreter.cpp]
  ...
  set(_minimal_src_files
      ...
      "src/script/shiftnum.cpp"
      "src/script/script_flags.cpp"
...
function(bdkSetApplicationListBSVSource)########################################################################################
  if(NOT (DEFINED BDK_BSV_ROOT_DIR AND EXISTS "${BDK_BSV_ROOT_DIR}"))
      message(FATAL_ERROR "Unable to locate bsv source code for BDK_BSV_ROOT_DIR")
  endif()

  set(_application_hdr_files
  ...
  foreach(rel_file ${_application_hdr_files})
    bdkAppendToGlobalSet(BSV_APPLICATION_HDR_FILES "${BDK_BSV_ROOT_DIR}/${rel_file}")
  endforeach()

  set(_application_src_files
    "src/config.cpp"
    "src/core_read.cpp"
    "src/core_write.cpp"
    "src/dstencode.cpp"
  ...
  foreach(rel_file ${_application_src_files})
    bdkAppendToGlobalSet(BSV_APPLICATION_SRC_FILES "${BDK_BSV_ROOT_DIR}/${rel_file}")
  endforeach()
endfunction() ## bdkSetMinimumListBSVSource
...
function(bdkPrintBSVSourceInfo)
  ...
  bdkPrintList("BSV_APPLICATION_HDR_FILES" "${BSV_APPLICATION_HDR_FILES}")
  bdkPrintList("BSV_APPLICATION_SRC_FILES" "${BSV_APPLICATION_SRC_FILES}")
