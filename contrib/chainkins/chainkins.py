...
if args.dump_pr_email_html is not None and args.dump_pr_email_html:
    ...
    CHAINKINS_PR_BITBUCKET_ACTOR = os.environ['CHAINKINS_PR_BITBUCKET_ACTOR'] if 'CHAINKINS_PR_BITBUCKET_ACTOR' in os.environ else 'Unknown'
    CHAINKINS_PR_BITBUCKET_TITLE = os.environ['CHAINKINS_PR_BITBUCKET_TITLE'] if 'CHAINKINS_PR_BITBUCKET_TITLE' in os.environ else 'Unknown'
    CHAINKINS_TARGET_REPO_HTTP = os.environ['CHAINKINS_TARGET_REPO_HTTP'] if 'CHAINKINS_TARGET_REPO_HTTP' in os.environ else 'Unknown'
    CHAINKINS_TARGET_BRANCH = os.environ['CHAINKINS_TARGET_BRANCH'] if 'CHAINKINS_TARGET_BRANCH' in os.environ else 'Unknown'
    CHAINKINS_TARGET_COMMIT = os.environ['CHAINKINS_TARGET_COMMIT'] if 'CHAINKINS_TARGET_COMMIT' in os.environ else 'Unknown'
    ...
    html_email_content=''
    html_email_content += 'Pull Request author : <b>{}</b><br><br>\n\n'.format(CHAINKINS_PR_BITBUCKET_ACTOR)
    html_email_content += 'Code review <a href={}>{}</a><br>\n'.format(jBITBUCKET_PULL_REQUEST_LINK, CHAINKINS_PR_BITBUCKET_TITLE)
    ...
    html_email_content += '<br>\nSource repository : {}<br>\n'.format(CHAINKINS_TARGET_REPO_HTTP)
    html_email_content += 'Source branch : {}<br>\n'.format(CHAINKINS_TARGET_BRANCH)
    html_email_content += 'Source commit : <a href={}/commits/branch/{}>{}</a><br>\n'.format(CHAINKINS_TARGET_REPO_HTTP,CHAINKINS_TARGET_BRANCH,CHAINKINS_TARGET_COMMIT)
...
if args.dump_mainrepo_email_html is not None and args.dump_mainrepo_email_html:
    ...
    CHAINKINS_TARGET_BRANCH = os.environ['CHAINKINS_TARGET_BRANCH'] if 'CHAINKINS_TARGET_BRANCH' in os.environ else 'Unable to define branch'
    CHAINKINS_BUILD_TRIGGER = os.environ['CHAINKINS_BUILD_TRIGGER'] if 'CHAINKINS_BUILD_TRIGGER' in os.environ else 'Unknown'
    jRUN_DISPLAY_URL = os.environ['RUN_DISPLAY_URL'] if 'RUN_DISPLAY_URL' in os.environ else 'Unknown URL'
    jBUILD_NUMBER = os.environ['BUILD_NUMBER'] if 'BUILD_NUMBER' in os.environ else 'Unknown'
    jJENKINS_SLAVE_OS = os.environ['JENKINS_SLAVE_OS'] if 'JENKINS_SLAVE_OS' in os.environ else 'Unknown'
    CHAINKINS_TARGET_REPO_HTTP = os.environ['CHAINKINS_TARGET_REPO_HTTP'] if 'CHAINKINS_TARGET_REPO_HTTP' in os.environ else 'Unknown URL'
    CHAINKINS_TARGET_COMMIT = os.environ['CHAINKINS_TARGET_COMMIT'] if 'CHAINKINS_TARGET_COMMIT' in os.environ else 'Unknown'
    jBUILD_URL = os.environ['BUILD_URL'] if 'BUILD_URL' in os.environ else 'Unknown URL'

    ### Building email content ################################################
    html_email_content=''
    html_email_content += 'Build branch [<b>{}</b>] triggered by : <i>{}</i><br><br>\n\n'.format(CHAINKINS_TARGET_BRANCH, CHAINKINS_BUILD_TRIGGER)
...
if args.update_bitbucket_build_status is not None and args.update_bitbucket_build_status:
    ...
    CHAINKINS_TARGET_BRANCH = os.environ['CHAINKINS_TARGET_BRANCH'] if 'CHAINKINS_TARGET_BRANCH' in os.environ else 'Unknown'
    CHAINKINS_PR_BITBUCKET_TITLE = os.environ['CHAINKINS_PR_BITBUCKET_TITLE'] if 'CHAINKINS_PR_BITBUCKET_TITLE' in os.environ else 'unknown pull request'
    ...
    if 'BITBUCKET_PULL_REQUEST_ID' in os.environ: ## This is a pull request build
        ...
        query_name = '__pr#{} : {}'.format(pr_id, CHAINKINS_PR_BITBUCKET_TITLE)
        ...
        query_desc = 'Build #{} branch [{}]'.format(jBUILD_NUMBER,CHAINKINS_TARGET_BRANCH)
        if 'CHAINKINS_BUILD_TRIGGER' in os.environ and 'Bitbucket' not in os.environ['CHAINKINS_BUILD_TRIGGER']:## build on main repo that's triggered manually
            query_desc +=' - triggered by {}'.format(os.environ['CHAINKINS_BUILD_TRIGGER'])
