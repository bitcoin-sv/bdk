...
def _dump_post_email_info(groovy_file) {
    if (!env.BITBUCKET_PAYLOAD) {// If BITBUCKET_PAYLOAD is not defined, then the build is manually triggered by a user
        ...
        sh label : "Dump common env variables for email", script : """
            ...
            echo env.CHAINKINS_EMAIL_TO_SEND=\\\"j.murphy@nchain.com,c.gibson@nchain.com, m.fletcher@nchain.com,m.prakash@nchain.com,c.nguyen@nchain.com,cc:s.okane@nchain.com,cc:r.balagourouche@nchain.com\\\">>${groovy_file}
        """
    }
}
